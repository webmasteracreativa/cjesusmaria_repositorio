	...
	<div id='primary' class="content-area">
		<div class='eventon site-main'>
			...
			<?php if($location_address):
				$CAL = new EVO_Calendar('evcal_1');
				$zoomlevel = $CAL->get_prop('evcal_gmap_zoomlevel');
					if(!$zoomlevel) $zoomlevel = 16;

				$map_type = $CAL->get_prop('evcal_gmap_format');
					if(!$map_type) $map_type = 'roadmap';
			?>
				<div id='evo_locationcard_gmap' class="evo_location_map" data-address='<?php echo $location_address;?>' data-latlng='<?php echo $location_latlan;?>' data-location_type='<?php echo $location_type;?>'data-zoom='<?php echo $zoomlevel;?>' data-scroll='<?php echo $CAL->is_yes('evcal_gmap_scroll')? 'no':'yes';?>' data-mty='<?php echo $map_type;?>'></div>
			<?php endif;?>
