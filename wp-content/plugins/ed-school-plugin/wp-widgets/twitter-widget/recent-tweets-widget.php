...
class TL_Recent_Tweets extends WP_Widget
{
	...
	public function __construct()
	{
		parent::__construct(
			...
			array('description' => __('Display recent tweets', $this->textdomain))
		);
	}
	...
	public function widget($args, $instance)
	{
		...
		if (empty($instance['consumerkey']) ||
			...
			echo '<strong>' . __('Please fill all required widget settings!', $this->textdomain) . '</strong>' . $after_widget;
			return;
		} else {
			...
			if ($diff >= $crt || empty($tl_twitter_last_cache_time)) {
				...
				if (!require_once('twitteroauth.php')) {
					echo '<strong>' . __('Couldn\'t find twitteroauth.php!', $this->textdomain) . '</strong>' . $after_widget;
					return;
				}
				...
				if (!$tweets) {
					echo '<strong>' . __('Unable to connect!', $this->textdomain) . '</strong>' . $after_widget;
					return;
				}

				if (!empty($tweets->errors)) {
					if ($tweets->errors[0]->message == 'Invalid or expired token') {
						echo '<strong>' . esc_html($tweets->errors[0]->message) . '!</strong><br />' . __('You\'ll need to regenerate it <a href="https://apps.twitter.com/" target="_blank">here</a>!', $this->textdomain) . $after_widget;
					} else {
		...
		if (!empty($tl_twitter_plugin_tweets) && is_array($tl_twitter_plugin_tweets)) {
			...
			print '<div class="tl-recent-tweets">
						' . __('<b>Error!</b> Couldn\'t retrieve tweets for some reason!', $this->textdomain) . '
				   </div>';
		}
	...
	public function form($instance)
	{
		...
		echo '
				<p>' . __('Get your API keys & tokens at', $this->textdomain) . ':<br />
				    <a href="https://apps.twitter.com/" target="_blank">https://apps.twitter.com/</a>
                </p>
				<p>
				    <label>' . __('Title:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Consumer Key:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Consumer Secret:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Access Token:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Access Token Secret:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Cache Tweets in every:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Twitter Username:', $this->textdomain) . '</label>
				...
				<p>
				    <label>' . __('Tweets to display:', $this->textdomain) . '</label>
		...
		echo '</select></p>
				<p>
				    <label>' . __('Exclude replies:', $this->textdomain) . '</label>
...
if (!function_exists('tl_relative_time')) {
	function tl_relative_time($a)
	{
		...
		if (is_numeric($d) && $d > 0) {
			...
			if ($d < 3) return __('right now', SCP_TEXT_DOMAIN);
			//if less then minute
			if ($d < $minute) return floor($d) . __(' seconds ago', SCP_TEXT_DOMAIN);
			//if less then 2 minutes
			if ($d < $minute * 2) return __('about 1 minute ago', SCP_TEXT_DOMAIN);
			//if less then hour
			if ($d < $hour) return floor($d / $minute) . __(' minutes ago', SCP_TEXT_DOMAIN);
			//if less then 2 hours
			if ($d < $hour * 2) return __('about 1 hour ago', SCP_TEXT_DOMAIN);
			//if less then day
			if ($d < $day) return floor($d / $hour) . __(' hours ago', SCP_TEXT_DOMAIN);
			//if more then day, but less then 2 days
			if ($d > $day && $d < $day * 2) return __('yesterday', SCP_TEXT_DOMAIN);
			//if less then year
			if ($d < $day * 365) return floor($d / $day) . __(' days ago', SCP_TEXT_DOMAIN);
			//else return more than a year
			return __('over a year ago', SCP_TEXT_DOMAIN);
		}
	}
}
