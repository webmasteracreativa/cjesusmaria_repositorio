...
if ( !class_exists( 'lshowcase_ordering' ) ) {
	class lshowcase_ordering {
		...
		public static function wp() {
			if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) ) {
				...
				wp_enqueue_script( 'lshowcase-simple-page-ordering', plugins_url( $script_name, __FILE__ ), array('jquery-ui-sortable'), '2.1', true );
				wp_enqueue_style( 'lshowcase-simple-page-ordering', plugins_url( 'js/order/simple-page-ordering.css', __FILE__ ) );	
			}
		...
		public static function admin_head() {
			...
			$screen->add_help_tab(array(
				...
				'title' => 'Orden simple de página',
				'content' => '<p>' . __( 'Para cambiar la posición de un elemento, simplemente arrastre y suelte la fila "haciendo clic y manteniéndolo" en cualquier lugar (fuera de los enlaces y controles de formulario) y moviéndolo a su nueva posición.', 'lshowcase' ) . '</p>',
			));
		}
		...
		public static function sort_by_order_link( $views ) {
			...
			if(get_query_var('orderby') != 'menu_order') {
				...
				$views['byorder'] = sprintf('<a href="%s" class="%s">%s</a>', $query_string, $class, __("Ordenar por", 'lshowcase'));
					

			} else {

				$views['byorder'] = __("Drag & Drop Activado", 'lshowcase');
...
lshowcase_ordering::get_instance();
