...
function dtbwp_envato_setup_logo_image( $old_image_url ) {
	return get_template_directory_uri() . '/lib/envato_setup/images/aislin.png';
}
				...
				public function theme_setup_wizard_content( $content ) {
					if ( $this->is_possible_upgrade() ) {
						array_unshift_assoc( $content, 'upgrade', array(
							'title'            => __( 'Upgrade', 'ed-school' ),
							'description'      => __( 'Upgrade Content and Settings', 'ed-school' ),
							'pending'          => __( 'Pending.', 'ed-school' ),
							'installing'       => __( 'Installing Updates.', 'ed-school' ),
							'success'          => __( 'Success.', 'ed-school' ),
