...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		protected $theme_prefix = 'ed_school_';
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'ed-school' ), esc_html__( 'Setup Wizard', 'ed-school' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'ed-school' ), esc_html__( 'Setup Wizard', 'ed-school' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
...
//			);
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'ed-school' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'ed-school' ) . '</ti' . 'tle>'; ?>
		...
		<h1 id="wc-logo">
			<a href="http://themeforest.net/user/aislin/portfolio" target="_blank"><?php
		...
		<a class="wc-return-to-dashboard"
		   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'ed-school' ); ?></a>
		<!--		--><?php //endif; ?>
		</body>
		<?php
	...
	<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'ed-school' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'ed-school' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'ed-school' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'ed-school' ); ?></a>
				</p>
			<?php
			} else if ( get_option( 'envato_setup_complete', false ) ) {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'ed-school' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'ed-school' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'ed-school' ); ?></a>
					</li>
<!--					<li>-->
<!--						<form method="post">-->
<!--							<input type="hidden" name="reset-font-defaults" value="yes">-->
<!--							<input type="submit" class="button-primary button button-large button-next"-->
<!--							       value="--><?php //esc_attr_e( 'Reset font style and colors', 'ed-school' ); ?><!--" name="save_step"/>-->
...
<!--					</li>-->
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'ed-school' ); ?></a>
				</p>
			<?php
			} else {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'ed-school' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'ed-school' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'ed-school' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'ed-school' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'ed-school' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_requirements() {
			...
			?>
			<h3><?php esc_html_e( 'Befeore proceeding, please watch installation video and read the requirements bellow', 'ed-school' ); ?></h3>
			<p>
				<a style="margin-right: 20px;" target="_blank" href="https://www.youtube.com/watch?v=Iu21NYL91Kc"><img
						src="<?php echo get_template_directory_uri() . '/assets/img/theme-install-video.jpg' ?>"
						alt=""/></a>
			</p>
			...
			<?php esc_html_e( 'Visual Composer and Revolution Slider plugins are included with the theme but you do not get purchase codes for these plugins.', 'ed-school' ); ?>
			</h4>
			<p>
		...
		<?php if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) : ?>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
			</p>
			<?php
		}
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'ed-school' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'ed-school' ); ?></p>
				...
				<?php
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'ed-school' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'ed-school' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'ed-school' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'ed-school' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'ed-school' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'ed-school' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'ed-school' ) ) );
			}
			exit;
		...
		public function envato_setup_rev_slider() {
			...
			require_once( get_template_directory() . '/lib/demo-importer/includes/Wheels_Revolution_Slider_Importer.php' );
			$rev_importer = new Wheels_Revolution_Slider_Importer();
			$rev_importer->cleanup();
			?>

			<form method="post" enctype="multipart/form-data">
				<p><?php esc_html_e( 'Please select Revolution slider to import', 'ed-school' ); ?></p>

				<input type="file" name="import_file">

				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'ed-school' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_rev_slider_handler() {
			...
			<form method="post">
				<p><?php esc_html_e( 'Please select which sliders to import', 'ed-school' ); ?></p>
					...
					<?php foreach ( $sliders as $slider ) : ?>
						...
						<li>
							<label for="<?php echo esc_attr( $slider ); ?>">
								...
								<?php echo esc_attr( $slider ); ?> <span></span>
							</label>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_rev_slider"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_rev_slider_cleanup() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['cleanup'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Error', 'ed-school' ) ) );
			}
		...
		public function ajax_rev_slider() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slider'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slider Found', 'ed-school' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'ed-school' ),
				) );
			}
			exit;
		}

		public function envato_setup_redux_options() {



		}

		public function envato_setup_redux_options_handler() {
			check_admin_referer( 'envato-setup' );
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'ed-school' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.', 'ed-school' ),
						'installing'       => esc_html__( 'Installing.', 'ed-school' ),
						'success'          => esc_html__( 'Success.', 'ed-school' ),
			...
			$content['widgets']  = array(
				'title'            => esc_html__( 'Widgets', 'ed-school' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'ed-school' ),
				'pending'          => esc_html__( 'Pending.', 'ed-school' ),
				'installing'       => esc_html__( 'Installing Default Widgets.', 'ed-school' ),
				'success'          => esc_html__( 'Success.', 'ed-school' ),
			...
			$content['settings'] = array(
				...
				'description'      => esc_html__( 'Configure default settings.', 'ed-school' ),
				'pending'          => esc_html__( 'Pending.', 'ed-school' ),
				'installing'       => esc_html__( 'Installing Default Settings.', 'ed-school' ),
				'success'          => esc_html__( 'Success.', 'ed-school' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content', 'ed-school' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'ed-school' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'ed-school' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e( 'Item', 'ed-school' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'ed-school' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'ed-school' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'ed-school' ) ) );
			}
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'ed-school' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'ed-school' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'ed-school' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'ed-school' ), size_format( $max_size ) ) );
				}
			}
		...
		public function _content_install_settings() {
			...
			foreach ( $redux_options as $r_option_name => $r_option_val ) {
				if (in_array($r_option_name, array(
					'header-layout-block',
			...
			update_option( ED_SCHOOL_THEME_OPTION_NAME, $redux_options );
			update_option( ED_SCHOOL_THEME_OPTION_NAME . '-transients', array( 'run_compiler' => 1 ) );
			...
			$page_meta_fields = array(
				$prefix . 'header_layout_block',
				$prefix . 'header_layout_block_mobile',
				$prefix . 'footer_layout_block',
				$prefix . 'quick_sidebar_layout_block',
			);
			foreach (get_all_page_ids() as $page_id) {

				foreach ($page_meta_fields as $page_meta_field) {
					$page_meta_val = get_post_meta($page_id, $page_meta_field, true);

					$inserted_id = $this->_imported_post_id( intval( $page_meta_val ) );
					if ( $inserted_id ) {
						update_post_meta( $page_id, $page_meta_field, $inserted_id );
					}
				}
			}
		...
		public function envato_setup_color_style() {
			...
			?>
			<h1><?php esc_html_e( 'Site Style', 'ed-school' ); ?></h1>
			<form method="post">
				<p><?php esc_html_e( 'Please choose your site style below.', 'ed-school' ); ?></p>
						...
						foreach ( $this->site_styles as $style_name => $style_data ) {
							...
							<li<?php echo $style_name == $current_style ? ' class="current" ' : ''; ?>>
								<a href="#" data-style="<?php echo esc_attr( $style_name ); ?>"><img
										src="<?php echo esc_url( get_template_directory_uri() . '/lib/envato_setup/images/' . $style_name . '/style.jpg' ); ?>"></a>
							</li>
						<?php } ?>
					</ul>
				</div>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo', 'ed-school' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'ed-school' ), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'ed-school' ); ?></a>
						</td>
					</tr>
				</table>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'ed-school' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
						</p>
					<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'ed-school' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
							<em>On the next page you will be asked to Login with your ThemeForest account and grant
								permissions to enable Automatic Updates. If you have any questions please <a
									href="https://themeforest.net/user/aislin" target="_blank">contact us</a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'ed-school' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_customize() {
			...
			<p>
				Most changes to the website can be made through the Theme Options dashboard.
			</p>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'ed-school' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="https://themeforest.net/user/aislin" target="_blank">https://themeforest.net/user/aislin</a>
				and includes:</p>
			<ul>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'ed-school' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		<?php
		}
		...
		public function envato_setup_ready() {
			...
			<a href="https://twitter.com/share" class="twitter-share-button"
			   data-url="https://themeforest.net/user/aislin/portfolio?ref=aislin"
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'ed-school' ); ?></h1>
			...
			<p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. <br/>Follow <a href="https://twitter.com/aislinthemes"
				                                                 target="_blank">@aislinthemes</a>
				on Twitter to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'ed-school' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="https://twitter.com/aislinthemes"
						                             target="_blank"><?php esc_html_e( 'Follow @aislinthemes on Twitter', 'ed-school' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'ed-school' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'ed-school' ); ?></h2>
					<ul>
						<!--						<li class="documentation"><a href="http://dtbaker.net/envato/documentation/"-->
						<!--						                             target="_blank">-->
						<?php //esc_html_e( 'Read the Theme Documentation', 'ed-school' ); ?><!--</a>-->
						<!--						</li>-->
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'ed-school' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'ed-school' ); ?></a></li>
						<li class="support"><a href="https://themeforest.net/user/aislin"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'ed-school' ); ?></a></li>
					</ul>
				</div>
			</div>
		<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class         = 'error';
					echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'ed-school' ), $error_message ) . '</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'ed-school' ), $this->envato_username ),
			...
			add_settings_field(
				...
				esc_html__( 'oAuth Login', 'ed-school' ),
				...
				if ( ! $option || empty( $option['oauth'] ) || empty( $option['oauth'][ $this->envato_username ] ) || empty( $envato_items ) || empty( $envato_items['items'] ) || ! envato_market()->items()->themes( 'purchased' ) ) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime( '-7 days' ) ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
								_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.', 'ed-school' );
								?></p>

							<p>
								<?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>', 'ed-school' ), esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
							</p>
						</div>
